...
interface ProfilePageProps {
  profile: ApiProfile;
  onUpdateProfile: (payload: Partial<ApiProfile>) => void;
}
...
export function ProfilePage({ profile, onUpdateProfile }: ProfilePageProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        onUpdateProfile({ avatar: nextAvatar });
  ...
  const handlePreferenceChange = (key: keyof ApiProfile['preferences']) => {
    const nextPreferences = { ...preferences, [key]: !preferences[key] };
    setPreferences(nextPreferences);
    onUpdateProfile({ preferences: nextPreferences });
  };
      ...
      <div className="max-w-4xl mx-auto px-8 py-8">
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <div className="relative group cursor-pointer" onClick={handleAvatarClick}>
            ...
            <div
              className="absolute -inset-2 border-[2px] border-[#1a1a1a] rounded-full animate-pulse opacity-50"
            ...
            {!isEditingName ? (
              <>
                <div className="flex items-center justify-center gap-2 group">
                  <h1 className="text-3xl font-bold handwritten mb-1">{profile.userName}</h1>
                  <button
                ...
                <p className="text-[#6d6d6d] font-medium handwritten text-lg">{profile.userHandle}</p>
              </>
            ) : (
          ...
          <div className="bg-[#faf8f3] border-[2.5px] border-[#1a1a1a] p-6 hand-drawn-border wireframe-shadow flex flex-col">
            ...
            <div className="flex-1 flex flex-wrap justify-center items-center content-center gap-4 min-h-[250px] p-4">
              {bubbles.map((bubble) => (
                ...
                >
                  <span className="font-bold handwritten text-[12px] select-none text-center break-words px-1 leading-tight w-full max-w-[85%]">
                    {bubble.label}
                  </span>
                </motion.div>
              ))}
            ...
            <div className="flex items-center justify-between py-3 border-b border-[#d4cdb8]">
              ...
              <label className="relative inline-block w-12 h-6 cursor-pointer">
                <input
                  ...
                  onChange={() => handlePreferenceChange('saveHistory')}
                />
                <div className="w-full h-full border-[2.5px] border-[#1a1a1a] bg-[#faf8f3] peer-checked:bg-[#1a1a1a] transition-colors hand-drawn-border"></div>
                <div className="absolute top-0.5 left-0.5 w-4 h-4 bg-[#1a1a1a] peer-checked:bg-[#faf8f3] transition-all peer-checked:translate-x-6 border-[1.5px] border-[#1a1a1a]"></div>
              </label>
